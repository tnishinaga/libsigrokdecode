...
ir = {
    ...
    '1110': 'IDCODE', # ID code register
    '1010': 'DPACC',  # Debug port access register
    '1011': 'APACC',  # Access port access register
    '1000': 'ABORT',  # Abort register # TODO: 32 bits? Datasheet typo?
}

# Boundary scan data registers (in IR[8:4]) and their sizes (in bits)
bs_ir = {
    '11111': 'BYPASS', # Bypass register
}

# ARM Cortex-M3 r1p1-01rel0 ID code
cm3_idcode = 0x4ba00477
...
def data_in(instruction, bits):
    ...
    if reg == "DP SELECT":
        tmp = [data[:-23], data[-8:-4], data[-4:]]
        tmp = list(map(lambda x: int('0b' + x, 2) ,tmp))
        apsel, apbanksel, dpbanksel = tmp[0], tmp[1], tmp[2]
        return 'New transaction: apsel 0x%x, apbanksel: 0x%x, dpbanksel: 0x%x, A: %s, RnW: %s' % (apsel, apbanksel, dpbanksel, reg, r)
    else:
...
class Decoder(srd.Decoder):
    ...
    id = 'jtag_dap'
    name = 'JTAG / ARM-DAP'
    longname = 'Joint Test Action Group / ARM DAP'
    desc = 'ARM DAP JTAG protocol.'
    license = 'gplv2+'
    inputs = ['jtag']
    outputs = []
    tags = ['Debug/trace']
